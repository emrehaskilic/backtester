...
for offset, label in offsets:
    t0 = time.time()
    r = rust_engine.run_candle_miner_py(c, h, l, bv, sv, oi, offset, BP_1D)
    elapsed = time.time() - t0
    all_results[offset] = (r, label, elapsed)
    print(f"  {label}: {elapsed:.2f}s | H:{r['high_total']} L:{r['low_total']}")

print()

# Quantile
print("QUANTILE MONOTONLUK (1D, 5Y)")
print("=" * 110)
...
# Ozet
print()
print("OZET: EN IYI PATTERN RATE BY OFFSET (1D, 5Y)")
print("=" * 110)
