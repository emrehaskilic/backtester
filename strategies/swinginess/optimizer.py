...
def pre_aggregate_bars(df):
    """Tick verisini 1-saniyelik bar'lara aggregate et (tamamen numpy vektörize)."""
    ts_sec = df["timestamp"].values // 1000
    price = df["price"].values.astype(np.float64)
    qty = df["quantity"].values.astype(np.float64)
    is_buy = (df["side"].values == "BUY")

    # Pandas groupby ile hizli aggregation
    df_agg = pd.DataFrame({
        "ts": ts_sec, "price": price, "qty": qty, "is_buy": is_buy,
        "buy_qty": qty * is_buy, "sell_qty": qty * (~is_buy),
    })

    grouped = df_agg.groupby("ts", sort=True)
    bars_ts = np.array(list(grouped.groups.keys()), dtype=np.int64)
    ...
    return {
        "ts": bars_ts,
        "open": agg["open"].values,
        "high": agg["high"].values,
        "low": agg["low"].values,
        "close": agg["close"].values,
        "buy_vol": agg["buy_vol"].values,
        "sell_vol": agg["sell_vol"].values,
        "trades": agg["trades"].values.astype(np.int64),
        "burst": np.zeros(len(bars_ts), dtype=np.int64),  # simplified
    }


def run_trial_from_bars(bars, params):
    """Bar verisinden backtest calistir (cok daha hizli)."""
    import math
    ...
    # Inline engine state (fonksiyon cagrisi overhead'ini azaltmak icin)
    delta_vals = deque(maxlen=rolling_window)
    cvd_slope_vals = deque(maxlen=rolling_window)
    logp_vals = deque(maxlen=rolling_window)
    obi_vals = deque(maxlen=rolling_window)
    sweep_vals = deque(maxlen=rolling_window)
    burst_vals = deque(maxlen=rolling_window)
    vol_vals = deque(maxlen=rolling_window)

    cvd = 0.0
    cvd_history = deque(maxlen=30)
    warmup_count = 0
    vwap_cum_pv = 0.0
    vwap_cum_v = 0.0

    # DFS components dict (engine uyumlulugu icin)
    class FakeEngine:
        def __init__(self):
            self.dfs_components = {
    ...
    for i in range(n_bars):
        buy_vol = bars["buy_vol"][i]
        sell_vol = bars["sell_vol"][i]
        ...
        cvd_slope = cvd_history[-1] - cvd_history[0] if len(cvd_history) >= 2 else 0

        log_pressure = 0
        if buy_vol > 0 and sell_vol > 0:
            log_pressure = math.log(buy_vol / sell_vol)

        sweep = 0
        op = bars["open"][i]
        if op > 0:
            sweep = (bars["close"][i] - op) / op * 10000

        burst_count = int(bars["burst"][i])

        vol = 0
        if op > 0:
            vol = (bars["high"][i] - bars["low"][i]) / op * 10000

        obi_approx = delta / (total_vol + 1e-10)

        delta_vals.append(delta)
        cvd_slope_vals.append(cvd_slope)
        logp_vals.append(log_pressure)
        sweep_vals.append(sweep)
        burst_vals.append(burst_count)
        vol_vals.append(vol)
        obi_vals.append(obi_approx)

        warmup_count += 1

        # VWAP
        mid = (bars["high"][i] + bars["low"][i] + bars["close"][i]) / 3
        vwap_cum_pv += mid * total_vol
        vwap_cum_v += total_vol
        engine.vwap = vwap_cum_pv / vwap_cum_v if vwap_cum_v > 0 else mid

        if warmup_count < 60:
            continue

        engine.warmup_done = True
        engine.prints_per_sec = int(bars["trades"][i])
        engine.last_price = float(bars["close"][i])

        engine.dfs_components["zDelta"] = _zscore(delta_vals, delta)
        engine.dfs_components["zCvd"] = _zscore(cvd_slope_vals, cvd_slope)
        engine.dfs_components["zLogP"] = _zscore(logp_vals, log_pressure)
        engine.dfs_components["zObiW"] = _zscore(obi_vals, obi_approx)
        engine.dfs_components["zObiD"] = _zscore(obi_vals, obi_approx) * 0.8
        engine.dfs_components["sweepSigned"] = _zscore(sweep_vals, sweep)
        engine.dfs_components["burstSigned"] = _zscore(burst_vals, burst_count)
        engine.dfs_components["oiImpulse"] = 0

        strategy.on_second(int(ts_arr[i]), float(bars["close"][i]), engine)

        if strategy.equity <= 0:
            break

    if strategy.position != 0:
        strategy._close_position(float(bars["close"][-1]), int(ts_arr[-1]), "CLOSE")

    return strategy.get_results()


def run_trial(df, params):
    """Tek bir parametre seti ile backtest calistir."""
    rolling_window = params.get("rolling_window_sec", 3600)
...
def objective(trial, bars):
    """Optuna objective fonksiyonu (bar-based)."""
    params = {
        # DFS ağırlıkları
    ...
    result = run_trial_from_bars(bars, params)
...
def optimize_pair(symbol, n_trials_override=None):
    n = n_trials_override or N_TRIALS
    print(f"\n{'='*60}")
    print(f"  {symbol} OPTIMIZASYONU ({n} trial)")
    print(f"{'='*60}")
    ...
    print(f"  Pre-aggregating bars...")
    in_bars = pre_aggregate_bars(in_sample)
    out_bars = pre_aggregate_bars(out_sample)
    print(f"  IS bars: {len(in_bars['ts']):,} | OOS bars: {len(out_bars['ts']):,}")
    del in_sample  # tick verisini serbest birak
    ...
    study.optimize(lambda trial: objective(trial, in_bars), n_trials=n, show_progress_bar=True)
    ...
    for rank, trial in enumerate(top_trials, 1):
        ...
        oos = run_trial_from_bars(out_bars, params)
...
def main():
    ...
    print("=" * 60)
    print("  SWINGINESS TICK REPLAY OPTIMIZASYON")
    ...
    # Özet rapor
    print(f"\n\n{'='*60}")
    print("  OZET -- EN IYI ROBUST SONUCLAR")
    print(f"{'='*60}")

    for symbol in SYMBOLS:
        if not all_results.get(symbol):
            print(f"\n  {symbol}: Sonuc bulunamadi")
            continue
        ...
        if robust:
            ...
            print(f"\n  {symbol}: Robust sonuc bulunamadi")

    print(f"\nSonuclar: {results_path}")


if __name__ == "__main__":
    # Tek pair: python optimize.py BTCUSDT [n_trials]
    if len(sys.argv) > 1:
        symbol = sys.argv[1].upper()
        n_trials = int(sys.argv[2]) if len(sys.argv) > 2 else N_TRIALS
        os.makedirs(RESULTS_DIR, exist_ok=True)
        results = optimize_pair(symbol, n_trials_override=n_trials)
