/// Sweep Candle Pattern Miner 15M — Quantile bazli pattern discovery
/// 15M mum kapanisi bazli continuation/reversal uzerinde.

use rayon::prelude::*;
use crate::sweep_candle_analysis_15m::{run_candle_analysis_15m, CandleEvent15m, CandleSignal15m, FEATURE_NAMES_15M};
...
pub struct CandleMinerResult15m {
    ...
    pub quantile_rows: Vec<QuantileRow15m>,

    // Top patterns
    pub high_cont_patterns: Vec<CandlePattern15m>,
    pub high_rev_patterns: Vec<CandlePattern15m>,
    pub low_cont_patterns: Vec<CandlePattern15m>,
    pub low_rev_patterns: Vec<CandlePattern15m>,
}

#[derive(Clone, Debug)]
pub struct QuantileRow15m {
...
fn mine_one_side(
    events: &[&CandleEvent15m],
    _is_cont_fn: &dyn Fn(&CandleSignal15m) -> bool,
    is_target_fn: &dyn Fn(&CandleSignal15m) -> bool,
    sweep_type: &str,
    target_name: &str,
) -> (f64, usize, Vec<QuantileRow15m>, Vec<CandlePattern15m>) {
    ...
    let mut q_rows: Vec<QuantileRow15m> = Vec::new();
    for fi in 0..7 {
    ...
    let mut candidates: Vec<CandlePattern15m> = combos.par_iter().filter_map(|(feats, quants)| {
...
pub fn run_candle_mining_15m(
    closes: &[f64], highs: &[f64], lows: &[f64],
    buy_vol: &[f64], sell_vol: &[f64], oi: &[f64],
) -> CandleMinerResult15m {
    // Run candle analysis first
    let analysis = run_candle_analysis_15m(closes, highs, lows, buy_vol, sell_vol, oi);

    // Split events by sweep type
    let high_events: Vec<&CandleEvent15m> = analysis.events.iter().filter(|e| {
        matches!(e.signal, CandleSignal15m::HighContinuation | CandleSignal15m::HighReversal)
    }).collect();

    let low_events: Vec<&CandleEvent15m> = analysis.events.iter().filter(|e| {
        matches!(e.signal, CandleSignal15m::LowContinuation | CandleSignal15m::LowReversal)
    }).collect();

    // Mine high sweep: continuation patterns
    let (hc_base, hc_total, hc_qrows, hc_patterns) = mine_one_side(
        &high_events,
        &|s| matches!(s, CandleSignal15m::HighContinuation),
        &|s| matches!(s, CandleSignal15m::HighContinuation),
        "high", "continuation",
    );
    ...
    let (_, _, _, hr_patterns) = mine_one_side(
        &high_events,
        &|s| matches!(s, CandleSignal15m::HighReversal),
        &|s| matches!(s, CandleSignal15m::HighReversal),
        "high", "reversal",
    );
    ...
    let (lc_base, lc_total, lc_qrows, lc_patterns) = mine_one_side(
        &low_events,
        &|s| matches!(s, CandleSignal15m::LowContinuation),
        &|s| matches!(s, CandleSignal15m::LowContinuation),
        "low", "continuation",
    );
    ...
    let (_, _, _, lr_patterns) = mine_one_side(
        &low_events,
        &|s| matches!(s, CandleSignal15m::LowReversal),
        &|s| matches!(s, CandleSignal15m::LowReversal),
        "low", "reversal",
    );
