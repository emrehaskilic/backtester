"""
CVD + OI WF Optimizer V2 — genisletilmis aralik.
5dk bar. 5 param. 1500 trial/fold.
"""
...
def main():
    print("=" * 80)
    print("  CVD + OI WF OPTIMIZER V2 — Genisletilmis Aralik")
    print("  5dk | 5 param | 1500 trial/fold")
    print("=" * 80)
    ...
    print(f"\n{'='*80}")
    print(f"  KONSENSUS V2")
    print(f"{'='*80}")
    ok = [r for r in all_results if r["status"] == "OK"]
    print(f"  Basarili: {len(ok)}/{nf}")

    if not all_params:
        print("  BASARISIZ."); return
    ...
    # V1 karsilastirma
    print(f"\n  V1 vs V2")
    print(f"  V1: cvd_period=16, threshold=0.10, 26/30 karli (87%), 4 trade/fold")
    print(f"  V2: cvd_period={consensus.get('cvd_period','?')}, threshold={consensus.get('cvd_threshold','?')}, "
          f"{profitable}/{len(ok)} karli ({profitable/len(ok)*100:.0f}%), {avg_trades:.0f} trade/fold")

    out = {"method": "CVD+OI WF V2", "consensus": consensus, "folds": nf, "ok": len(ok),
           "results": all_results, "params": all_params}
    op = os.path.join(RESULTS_DIR, "cvd_oi_wf_v2_results.json")
