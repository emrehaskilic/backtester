"""
KAMA 4 Katman Tam Konsensus — 1 Senelik Haftalik Rapor.
KAMA + KC(i-1) + Graduated DCA + Graduated TP
"""
...
DCA_M = [0.25, 0.25, 0.50, 1.00]
TP_P = [0.30, 0.45, 0.70, 0.80]
MAX_DCA = 4
INITIAL = 1000.0
...
def main():
    ...
    print()
    print("  " + "=" * 64)
    print("  SIGMA KAPITAL — KAMA 4 KATMAN TAM KONSENSUS")
    print("  " + "=" * 64)
    print(f"  ETHUSDT.P | 3m | 25x | $1,000 | Margin: kasa/40")
    print(f"  KAMA({KAMA_P},{KAMA_F},{KAMA_S}) Slope({SLOPE_LB},{SLOPE_TH})")
    print(f"  KC({KC_L},{KC_M},{KC_A}) kc[i-1]")
    print(f"  DCA{DCA_M} | TP{TP_P}")
    print("  " + "=" * 64)
    ...
    for week in weeks:
        ...
        results.append({"num": week["num"], "range": week["range"], "month": week["month"],
                         ...
                         "fees": round(r["total_fees"],2), "tp": r["tp_count"], "rev": r["rev_count"]})
    ...
    print(f"\n  KARSILASTIRMA\n  {'-'*50}")
    print(f"  {'':20s} | {'Saf KAMA':>10s} | {'4 Katman':>10s}")
    print(f"  {'-'*50}")
