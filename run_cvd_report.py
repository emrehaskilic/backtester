"""
SAF CVD Order Flow — 1 Senelik Haftalik Detayli Rapor.
30s mikro-mum uzerinde. DCA/TP yok.
"""
...
def main():
    ...
    warmup = 90 * 2880  # 90 gun x 2880 bar/gun
    ...
    print()
    print("  " + "=" * 64)
    print("  SIGMA KAPITAL — SAF CVD ORDER FLOW HAFTALIK RAPOR")
    print("  " + "=" * 64)
    print(f"  ETHUSDT.P | 30s mikro-mum | 25x | $1,000 | kasa/40")
    print(f"  CVD(period={CVD_PERIOD}, imb_weight={IMB_WEIGHT}, threshold={SIGNAL_TH})")
    print(f"  DCA/TP/Filtre: KAPALI — Saf order flow sinyali")
    print(f"  Veri: {len(df):,} bar (30s) | 2 ay warmup + 9 ay aktif")
    print("  " + "=" * 64)
    ...
    for week in weeks:
        ...
        if we <= ws: continue
        ss = max(0, ws - 5000)
        r = rust_engine.run_cvd_backtest(
            sa(c[ss:we]), sa(bv[ss:we]), sa(sv[ss:we]),
            CVD_PERIOD, IMB_WEIGHT, SIGNAL_TH)
        net = r["net_pct"]
    ...
    print(f"  {'':20s} | {'Saf KAMA':>10s} | {'Saf CVD':>10s}")
    print(f"  {'-'*50}")
    ...
    print(f"  {'Kar/Zarar':20s} | {'1.36x':>10s} | {abs(aw/al):.2f}x" if al != 0 else "")
